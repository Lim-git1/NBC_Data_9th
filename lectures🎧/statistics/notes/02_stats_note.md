...
### 3-3. 정규화(Normalization) vs 표준화(Standardization)

| 구분 | 정규화 (Normalization) | 표준화 (Standardization) |
|------|--------------------------|----------------------------|
| **목적** | 서로 다른 단위의 데이터를 **0~1 사이**로 맞추기 | 서로 다른 단위의 데이터를 **평균 0, 표준편차 1**로 맞춤 |
| **결과 범위** | 0 ~ 1 (또는 -1 ~ 1) | 평균 0, 표준편차 1 |
| **데이터 형태** | 값의 비율은 유지하지만, 이상치로 인해 **압축**될 수 있음 | 중심(평균) 이동 + 스케일(분산) 조정 → 상대적 거리 유지 |
| **이상치(Outlier)** | **민감함** → max/min에 영향 받음 | **비교적 둔감** → 평균·표준편차로 조정 |
| **대표 방법** | **Min-Max Scaling**  <br>→ (x - min) / (max - min) | **Z-score Scaling**  <br>→ (x - mean) / std |
| **적용 예시** | 신경망 입력(픽셀값, 확률 등)처럼 값의 범위가 제한될 때 | 거리 기반 알고리즘(KMeans, KNN), PCA, 회귀, SVM 등 대부분의 ML |
| **한줄 요약** | “값의 **범위를 동일하게** 맞춘다.” | “값의 **스케일(단위)를 동일하게** 맞춘다.” |
