...
input: 검정통계량 -> 확률분포(정규/t/X^2 등)에 대입 -> output: p-value
...
> 검정통계량 계산 → 확률분포에서 확률(p-value) 확인 → 유의수준과 비교 → 결론 도출
...
1. **일반 T-test (Student’s T-test)**

   - 두 집단의 분산이 같다고 가정 `equal_var=True`

   - 분산을 하나의 공통분산으로 합쳐 사용

2. **Welch’s T-test**

   - 두 집단의 분산이 다를 수 있다고 가정 `equal_var=False`
...
- 범주형 변수가 2개만 있는 경우 -> **이항검정**

- 범주형 변수가 여러개 존재하는 경우 -> **카이제곱**
...
> **분산분석(ANOVA)**
> 
> → 세 집단 이상에서 “평균이 모두 같은가?”를 검정
> 
> → 귀무가설: μ₁ = μ₂ = μ₃ = …
> 
> 결과가 유의하다면 (p < 0.05)
> 
>   → “적어도 한 집단은 다르다”는 것까지만 알 수 있음
> 
>   → 어느 집단이 다른지는 모름 ❌
>
> 그래서 필요한 게 → **다중비교**
>
>   → “어떤 집단 쌍이 유의하게 다른가?”를 알아보기 위한 사후검정
> >
> > **다중비교** (Multiple Comparison Test)
> >
> > 여러 집단의 평균을 쌍(pair) 단위로 비교할 때, **유의수준이 누적되어 생기는 오류(제1종 오류)** 를 제어하기 위한 방법.
> > 대표적인 다중비교 방법
> > 1. 본페로니 검정
> >
> > : 여러 번 검정하면 오류 확률이 누적되니까 → 유의수준 α를 비교 횟수 k로 나눠서 보수적으로 조정
> >
> > 2. 튜키 검정
> >
> > : 모든 집단 쌍을 동시에 비교하되, 전체 유의수준(α)을 유지하면서 개별 비교를 수행
...
> - 신뢰구간이 기준값(μ₀)을 포함한다 → p > 0.05 → 귀무가설 유지
> 
> - 신뢰구간이 기준값(μ₀)을 포함하지 않는다 → p < 0.05 → 귀무가설 기각
...
## 🧮 가설검정 통계 기법 요약표 (from.GPT)

| 구분 | 사용 상황 | 귀무가설(H₀) | 검정통계량 | 사용하는 분포 | 결과 해석 |
|------|-------------|----------------|----------------|----------------|----------------|
| **① z-검정 (Z-test)** | 모집단 분산(σ²)을 알고 있고, 표본이 충분히 클 때 | 모집단 평균 μ는 특정값과 같다 | Z = (x̄ - μ) / (σ / √n) | **정규분포** | p < 0.05 → 평균이 유의하게 다르다 |
| **② t-검정 (T-test)** | 모집단 분산 모를 때, 표본 작을 때 (n < 30) | 모집단 평균 μ는 특정값과 같다 | t = (x̄ - μ₀) / (s / √n) | **t분포** | p < 0.05 → 평균 차이가 유의하다 |
| **③ 독립표본 t-검정** | 두 집단(남/여, A/B 등)의 평균 비교 | 두 집단의 평균이 같다 | t = (x̄₁ - x̄₂) / SE | **t분포** | p < 0.05 → 두 집단 평균 유의하게 다름 |
| **④ 대응표본 t-검정** | 같은 집단의 전후 변화 비교 (before/after) | 변화량 평균이 0이다 | t = (d̄ - 0) / (sd / √n) | **t분포** | p < 0.05 → 전후 변화 유의 |
| **⑤ ANOVA (분산분석)** | 세 집단 이상 평균 비교 | 모든 집단의 평균이 같다 | F = 집단간분산 / 집단내분산 | **F분포** | p < 0.05 → 적어도 한 집단 평균이 다름<br>→ 사후검정(Tukey 등) 필요 |
| **⑥ 카이제곱 검정 (χ²-test)** | 범주형 변수 간 관계 검정 | 두 변수는 독립이다 | χ² = Σ((관측-기대)² / 기대) | **χ²분포** | p < 0.05 → 변수 간 연관성 있음 |
| **⑦ 적합도 검정 (Goodness-of-fit)** | 관측분포가 이론분포와 같은지 검정 | 관측분포 = 기대분포 | χ² = Σ((관측-기대)² / 기대) | **χ²분포** | p < 0.05 → 분포가 다름 |
| **⑧ 분산동질성 검정 (Levene, Bartlett)** | 두 집단 이상 분산이 같은지 확인 | 분산이 같다 | F 또는 W 통계량 | **F분포** | p < 0.05 → 분산 다름 → Welch t-test 사용 |
| **⑨ 비모수 검정 (Mann-Whitney, Wilcoxon)** | 정규성 가정 불만족 시 | 두 집단 중앙값이 같다 | 순위 기반 통계량 | **비모수 분포** | p < 0.05 → 중앙값 차이 유의 |
| **⑩ 상관분석 (Pearson / Spearman)** | 두 수치형 변수의 연관성 확인 | 상관계수 ρ = 0 | t = r√(n−2)/(√1−r²) | **t분포** | p < 0.05 → 유의한 상관관계 존재 |
| **⑪ 회귀분석 계수 검정** | 회귀모델에서 변수 영향력 검증 | 계수 β = 0 | t = β̂ / SE(β̂) | **t분포** | p < 0.05 → 해당 변수가 유의미한 영향 |
...
| 단계 | 설명 | 예시 |
|------|------|------|
| **① 가설 설정** | 귀무가설(H₀)과 대립가설(H₁)을 명확히 세운다 | H₀: 남녀 평균키 동일<br>H₁: 남녀 평균키 다름 |
| **② 검정통계량 계산** | 데이터로부터 검정통계량(t, F, χ² 등)을 계산 | 예: t = 2.45 |
| **③ p-value 계산** | 분포(t, F, χ² 등)에서 해당 값의 확률을 계산 | 예: p = 0.018 |
| **④ 유의수준(α)와 비교** | 보통 α = 0.05 사용 | 0.018 < 0.05 → 기각 ✅ |
| **⑤ 결론 도출** | 귀무가설 기각 여부로 해석 | “남녀의 평균키는 통계적으로 유의하게 다르다.” |
...
| 주의사항 | 설명 |
|-----------|------|
| **p-value < 0.05** | “우연히 이런 차이가 나올 확률이 5% 미만이다” = “차이가 유의하다” |
| **p-value > 0.05** | “귀무가설을 기각할 근거가 부족하다” (≠ 귀무가설이 참이다) |
| **통계적 유의 ≠ 실질적 의미** | 유의하다고 해서 항상 의미 있는 차이는 아님 (효과크기 확인 필요) |
| **전제조건 확인 필수** | 정규성, 독립성, 등분산성 등 가정 위반 시 → 비모수 검정 사용 |
...
| **ANOVA** | “세 집단 평균 비교” |
| **χ²-검정** | “범주형 변수 관계” |
| **F-검정** | “분산 비교 또는 회귀분석 유의성” |
| **z-검정** | “모집단 분산을 아는 평균 비교” |
| **비모수 검정** | “정규분포 가정 안 될 때 대체법” |
